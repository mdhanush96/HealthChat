...
def _safe_json_error(resp: requests.Response) -> str:
    """Return a human-readable error string from a non-2xx response.

    Tries to parse the response body as JSON first; falls back to the raw
    text or the HTTP status code so that a JSONDecodeError is never raised
    when the server returns an empty or non-JSON body.
    """
    try:
    ...
    with tab_register:
        ...
        if reg_submitted:
            ...
            if resp.status_code == 201:
                ...
                st.error(f"Registration failed: {_safe_json_error(resp)}")
...
def page_chat():
    ...
    with st.sidebar:
        ...
        hist_resp = api_get("/chat/conversations/")
        if hist_resp.status_code == 200:
            conversations = hist_resp.json()
            for conv in conversations:
                label = conv.get("title") or f"Conversation {conv['id']}"
                if st.button(label[:40], key=f"conv_{conv['id']}"):
                    st.session_state.conversation_id = conv["id"]
                    # Load messages for selected conversation
                    hist_resp2 = api_get(f"/chat/history/{conv['id']}/")
                    if hist_resp2.status_code == 200:
                        msgs = hist_resp2.json().get("messages", [])
                        st.session_state.messages = [
        ...
        if st.button("Analyse Report"):
            with st.spinner("Processing your report… this may take a moment."):
                ...
                if resp.status_code == 201:
                    ...
                    st.error(f"Upload failed: {_safe_json_error(resp)}")
